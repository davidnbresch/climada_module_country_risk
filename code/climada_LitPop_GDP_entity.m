...
if ~isfield(litpop,'lon')
    litpop.lon = litpop.gpw_lon(litpop.USA_ind);
end
if ~isfield(litpop,'lat')
    litpop.lat = litpop.gpw_lat(litpop.USA_ind);
end
...
if parameters.admin0_calc
    ...
    if parameters.do_agrar
        IN0_agrar = climada_inpolygon(GLB_agriculture.assets.lon,GLB_agriculture.assets.lat,shapes0(i0).X,shapes0(i0).Y,0);
    end
    ...
    % admin0_agriculture = GLB_agriculture;
        admin0.agriculture.assets.lon = GLB_agriculture.assets.lon(IN0_agrar==1);
        admin0.agriculture.assets.lat = GLB_agriculture.assets.lat(IN0_agrar==1);
        admin0.agriculture.assets.Value = GLB_agriculture.assets.Value(IN0_agrar==1);
    ...
    for i = 1:length(i1)
        ...
        if ~isempty(admin1_names{i1(i)})
            ...
            if parameters.admin1_calc
                ...
                litpop_tmp(IN_litpop==1) = litpop_tmp(IN_litpop==1)/nansum(litpop_tmp(IN_litpop==1)); % normalized by admin1 level
                if round(full(nansum(litpop_tmp(IN_litpop==1))).*1e9)~=1e9      
                    warning('nansum(litpop_tmp(IN_litpop==1)) is not equal to 1. Normalization failed.')
                else % distribute GSDP to gridpoints in state/ province (admin1): 
                    admin0.GDP.FromLitPop_admin1(IN_litpop==1) = litpop_tmp(IN_litpop==1).*admin1.GSDP_Reference(i); % multiply by reference GSDP
                end
                clear litpop_tmp
