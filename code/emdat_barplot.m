...
%   next call: many
% CALLING SEQUENCE:
%   legend_str=emdat_barplot(em_data,damage_symbol,damage_orig_symbol,legend_tag,legend_str,legend_location)
% EXAMPLE:
...
if ~exist('damage_symbol','var'),damage_symbol='db';end
if ~exist('damage_orig_symbol','var'),damage_orig_symbol='ob';end
if ~exist('legend_tag','var'),legend_tag='EM-DAT';end
if ~exist('legend_str','var'),legend_str={};end
if ~exist('legend_handle','var'),legend_handle=[];end
if ~exist('legend_location','var'),legend_location='SouthEast';end
...
if isfield(em_data,'DFC_orig')
    ...
    legend_str{end+1}   = [legend_tag ' orig'];
