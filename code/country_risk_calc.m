...
if isfield(country_risk.res,'hazard')
    ...
    for hazard_i=1:hazard_count
        ...
        % Note that for standard use, in the case the hazard event sets
        % have been generated by centroids_generate_hazard_sets, this is
        % not required, since the hazard sets have been just created for
        % the specific centroids (and the first n elements in the hazard 
        % are matching the n centroids, while the n+1... elements in hazard 
        % are the ones for the buffer around the country. 
        % The exception is WS Europe, where we use an existing hazard set, 
        % hence need to re-encode
        if strcmp(hazard.peril_ID,'WSEU')
            % re-encode to the respective centroids
