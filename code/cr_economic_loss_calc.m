...
for peril_i = 1:length(country_risk.res.hazard)
    if ~isempty(country_risk.res.hazard(peril_i).EDS)
        for damage_j = 1:length(country_risk.res.hazard(peril_i).EDS.damage)
            damage_per_GDP = country_risk.res.hazard(peril_i).EDS.damage(damage_j)/master_data.GDP_2013(country_index);
            loss_multiplier = 1+cr_get_damage_weight(damage_per_GDP) * country_damage_factor;
            country_risk_economic_loss.res.hazard(peril_i).EDS.damage(damage_j) = ...
                country_risk.res.hazard(peril_i).EDS.damage(damage_j) * loss_multiplier;
        end
    else
        country_risk_economic_loss.res.hazard(peril_i).EDS.damage = [];
    end
end
